    ...
    <div className="mt-2">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-1">
          <img className="h-5 w-5" src="/images/calendar.svg" alt="calendar icon" />
          <p className="font-poppins text-xs font-semibold">
            {currentDate.toLocaleString('default', { month: 'long' })} {currentDate.getFullYear()}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handlePrevWeek}><IoIosArrowBack className="h-4 w-4" /></button>
          <button onClick={handleNextWeek}><IoIosArrowForward className="h-4 w-4" /></button>
        </div>
      </div>
      ...
      <div className="flex mt-4 text-gray-400 font-semibold font-poppins text-xs">
        {weekDays.map((day, index) => (
          <button
            key={index}
            className="flex py-1 rounded px-1.5 flex-col hover:bg-orange-500 hover:text-white gap-1 items-center"
          >
            <p>{day.toLocaleString('default', { weekday: 'long' }).slice(0, 3)}</p>
            <p>{day.getDate()}</p>
          </button>
        ))}
      </div>
